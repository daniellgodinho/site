...
import {
    UserPlus, Trash2, LogOut, Clock, Calendar, Infinity,
    CheckCircle, XCircle, Search, Github
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { supabase } from '../supabase';  // ← Corrigido
...
export default function Dashboard() {
    ...
    // ... (todo o código do UserManager que eu enviei antes continua aqui exatamente igual)
    ...
    const exportToGitHub = async () => {
        ...
        try {
            const { data: allLists } = await supabase.from('user_lists').select('*');
            const allUsers = new Set();
            const allFarm = new Set();

            allLists.forEach(list => {
                if (list.users) list.users.split(',').forEach(str => {
                    const [username] = str.split('|');
                    if (username) allUsers.add(username.trim());
                });
                if (list.users_farm) list.users_farm.split(',').forEach(str => {
                    const [username] = str.split('|');
            ...
            const farmContent = Array.from(allFarm).join('\n');

            // Atualiza users
            const usersRes = await fetch(`https://api.github.com/repos/Aephic/dnmenu/contents/security/users`, {
                headers: { Authorization: `Bearer ${GITHUB_TOKEN}` }
            });
            const usersData = await usersRes.json();

            await fetch(`https://api.github.com/repos/Aephic/dnmenu/contents/security/users`, {
                method: 'PUT',
                headers: { Authorization: `Bearer ${GITHUB_TOKEN}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: 'Update users - DN Menu Manager',
            ...
            // Atualiza usersfarm
            const farmRes = await fetch(`https://api.github.com/repos/Aephic/dnmenu/contents/security/usersfarm`, {
                headers: { Authorization: `Bearer ${GITHUB_TOKEN}` }
            });
            const farmData = await farmRes.json();

            await fetch(`https://api.github.com/repos/Aephic/dnmenu/contents/security/usersfarm`, {
                method: 'PUT',
                headers: { Authorization: `Bearer ${GITHUB_TOKEN}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: 'Update usersfarm - DN Menu Manager',
                    content: btoa(unescape(encodeURIComponent(farmContent))),
                    sha: farmData.sha,
                    branch: 'main'
                })
            });
    ...
    const activeTokens = [...users, ...usersFarm].filter(u => !u.expiration || new Date(u.expiration) > new Date()).length;
    ...
    return (
        <div className="min-h-screen bg-black text-white relative overflow-hidden">
            ...
            <header className="relative z-10 bg-[#2e2e2e] border-b border-purple-600/20 shadow-xl">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                            <CustomSelect value={selectedReseller} onChange={setSelectedReseller} options={resellerOptions} className="w-48" />
                            <button onClick={exportToGitHub} className="flex items-center space-x-2 px-4 py-2 bg-[#2e2e2e] border border-purple-600/30 rounded-lg hover:bg-[#3a3a3a] transition">
                                <Github className="w-5 h-5" />
                            </button>
                            <button onClick={handleLogout} className="flex items-center space-x-2 px-4 py-2 bg-red-600/80 hover:bg-red-600 rounded-lg transition">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {saveStatus && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                        className={`p-4 rounded-xl mb-6 border ${saveStatus === 'salvo' ? 'bg-green-900/30 border-green-500/30 text-green-400' : saveStatus === 'erro' ? 'bg-red-900/30 border-red-500/30 text-red-400' : 'bg-blue-900/30 border-blue-500/30 text-blue-400'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    <motion.div whileHover={{ scale: 1.02 }} className="p-6 bg-[#2e2e2e] rounded-2xl border border-purple-600/20">
                ...
                <div className="flex space-x-4 mb-6">
                    <button onClick={() => setActiveTab('users')} className={`px-6 py-3 rounded-xl transition ${activeTab === 'users' ? 'bg-purple-600 shadow-lg shadow-purple-600/30' : 'bg-[#2e2e2e] border border-gray-700 hover:border-purple-600/40'}`}>
                        Users ({users.length})
                    </button>
                    <button onClick={() => setActiveTab('usersfarm')} className={`px-6 py-3 rounded-xl transition ${activeTab === 'usersfarm' ? 'bg-purple-600 shadow-lg shadow-purple-600/30' : 'bg-[#2e2e2e] border border-gray-700 hover:border-purple-600/40'}`}>
                ...
                <div className="mb-6 flex flex-col sm:flex-row gap-4">
                    <input
                        ...
                        className="flex-grow px-4 py-3 bg-[#2e2e2e] border border-purple-600/30 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-purple-600"
                    />
                    <CustomSelect
                        value={activeTab === 'users' ? selectedDuration : selectedDurationFarm}
                        onChange={(v) => activeTab === 'users' ? setSelectedDuration(v) : setSelectedDurationFarm(v)}
                ...
                <div className="mb-6 relative">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-3 bg-[#2e2e2e] border border-purple-600/30 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-purple-600"
                    />
                </div>

                <div className="overflow-x-auto rounded-2xl border border-purple-600/20 bg-[#2e2e2e]">
                    <table className="w-full">
                        <thead>
                            <tr className="bg-[#1a1a1a] border-b border-gray-700">
                                <th className="px-6 py-4 text-left text-gray-400 text-sm">Nome de Usuário</th>
                                <th className="px-6 py-4 text-left text-gray-400 text-sm">Duração</th>
                                <th className="px-6 py-4 text-left text-gray-400 text-sm">Tempo Restante</th>
                                <th className="px-6 py-4 text-left text-gray-400 text-sm">Status</th>
                                <th className="px-6 py-4 text-right text-gray-400 text-sm">Ações</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredUsers.map((user, i) => (
                                <motion.tr key={user.username} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="border-t border-gray-800 hover:bg-[#1a1a1a]">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => removeUser(activeTab, user.username)} className="p-2 hover:bg-red-500/20 rounded-lg transition border border-transparent hover:border-red-500/30">
